...
import { Paggination } from 'src/core/global/types/paggination';
...
export function getConditions<Query extends PaggingDTO, M extends Model>(query: Query, querySequelice: Omit<FindAndCountOptions<Attributes<M>>, 'group'>) {
  ...
  const keys = Object.keys(query).filter(value => !["rows", "populate", "search", "records", "page", "order", "deletedAt", "paranoid"].includes(value));
...
export function formatKeySearch(row: string, search: string, table_name: string) {
  ...
  const col_name = `${table_name.includes('.') ? '' : table_name}.${key}`
  switch (type) {
    case 'BOOLEAN':
      return literal(`CASE WHEN "${col_name}" THEN 'SI' ELSE 'NO' END ILIKE '%${search}%'`);
    default:
      return where(cast(col(col_name), 'varchar'), { [Op.iLike]: search })
  }
}
...
function transformerIncludes(elemt: any): FindOptions | Includeable {
  ...
  if (Array.isArray(elemt)) {
    ...
    if (typeof elemt === 'string') {
      ...
      throw new ForbiddenException('Error en la consulta de populate')
    }
  }
...
export function getIncludes<Query extends PaggingDTO>(query: Query): FindOptions {
  if (query.populate) {
    ...
    const includesPattern = JSON.parse(query.populate)
...
export default function getQuery<Query extends PaggingDTO, M extends Model>(query: Query, querySequelice: Omit<FindAndCountOptions<Attributes<M>>, 'group'>, paggination: Paggination, includes?: any) {
...
export function getPaggination<Query extends PaggingDTO>({records, page}: Query): Paggination {
